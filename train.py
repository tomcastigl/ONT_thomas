...
def evaluate_model(model, loader, device, n_gpu, loss_fn, global_step, run_name):
    model.eval()
    eval_loss = 0.0
    nb_eval_steps = 0
    correct = 0
    ...
    for batch in tqdm(loader, desc="Evaluating"):
        ...
        with torch.no_grad():
            ...
            pred_df=pd.DataFrame(output, columns=classes)
            y_pred_scores=y_pred_scores.append(pred_df)
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if args.evaluate_during_training and (global_step + 1) \
                ...
                evaluate_model(model, test_dataloader, args.device, args.n_gpu, loss_fn, global_step,args.run_name)
            global_step += 1
...
def main():
    ...
    with mlflow.start_run(run_name=args.run_name):
        ...
        if args.eval:
            model.eval()
            evaluate_model(model, val_dataloader, args.device, args.n_gpu, loss_fn, global_step,args.run_name)
