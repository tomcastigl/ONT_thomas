...
def evaluate_model(model, loader, device, n_gpu, loss_fn, global_step, run_name, threshold):
    ...
    for batch in tqdm(loader, desc="Evaluating"):
        ...
        with torch.no_grad():
            ...
            prob=th_delete(prob,idx_rm)
            predicted=th_delete(predicted,idx_rm)
            targets=th_delete(targets,idx_rm)
            drop_count+=len(idx_rm)
            #temp_df=pd.DataFrame()
            #temp_df['read_id_token']=batch[3].cpu().numpy().tolist()
            #temp_df['chunk_idx']=batch[4].cpu().numpy().tolist()
            #temp_df=temp_df.join(pd.DataFrame(torch.exp(pred.data).cpu().numpy()))
            #temp_df['pred']=predicted.cpu().numpy().tolist()
            #probas_df=probas_df.append(temp_df, ignore_index=True)
    ...
    #probas_df.set_axis(['read_id_token','chunk_idx','UNM','h5mC','pU','m5C','m6A','pred'],axis=1,inplace=True)
    #probas_df.to_csv('probas_df_tresh.csv')
    ...
    print(confmat)
    print(f'total samples: {total} dropped {drop_count} ({100*(drop_count/total)}% dropped)')
