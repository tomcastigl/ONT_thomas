...
class Conformer(nn.Module):
    ...
    def __init__(
        ...
        # )
        self.fc = Linear(decoder_dim, num_classes, bias=False)
        self.rnn = nn.LSTM(encoder_dim, decoder_dim, num_decoder_layers, dropout=decoder_dropout_p,
                           batch_first=True)
    ...
    def forward(
        ...
        out, out_lengths = self.encoder(inputs, input_lengths)
        # decoder_outputs, _ = self.decoder(targets, target_lengths)
        # outputs = self.joint(encoder_outputs, decoder_outputs)
        # pass through lstm
        # hidden = (torch.randn(1, 1, 3), torch.randn(1, 1, 3))  # clean out hidden state
        # packed_out = torch.nn.utils.rnn.pack_padded_sequence(encoder_outputs, out_lengths,
        #                                                      batch_first=True, enforce_sorted=False)
        self.rnn.flatten_parameters()
        out, _ = self.rnn(out)
        #print(f'out_lengths_2 {out_lengths[2]}')
        #print(f'out.dim={out.dim()}, out_lenghts={out_lengths}, decoder_dim={self.decoder_dim}')
        out = torch.gather(out, 1, (out_lengths - 1).view(-1, 1).unsqueeze(2).repeat(
            1, 1, self.decoder_dim)).squeeze(dim=1)
        #print(out.dim())
        return self.log_softmax(self.fc(out))  #change self.log_softmax to self.exp
    ...
    def decode(self, encoder_output: Tensor, max_length: int) -> Tensor:
        ...
        return decoder_input#torch.LongTensor(pred_tokens)
